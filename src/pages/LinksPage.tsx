...
export const renderSocialIcon = (iconType: string, className = "h-5 w-[#1.25rem]") => {
  switch (iconType) {
    case "instagram":
      return (
    ...
    case "bot":
      return (
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-teal-600 flex items-center justify-center text-white shadow-md shadow-cyan-500/20 group-hover:scale-105 transition-transform duration-300">
...
const LinksPage: React.FC<LinksPageProps> = ({ previewMode = false, customSettings }) => {
  ...
  return (
    <div
      className={`min-h-screen bg-[#071317] text-white selection:bg-emerald-500 selection:text-black flex flex-col items-center justify-between font-sans relative overflow-hidden ${
        previewMode ? "p-4 min-h-[580px] rounded-3xl" : "p-6 py-12"
      }`}
    >
      {/* Dynamic Background Pattern & Glow Effect */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-emerald-500/10 rounded-full blur-[120px]" />
        <div className="absolute -bottom-32 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-teal-500/10 rounded-full blur-[120px]" />
        <div
      ...
      {!previewMode && (
        <button
          onClick={handleShare}
          title="Nusxalash"
          className="absolute top-5 right-5 z-20 p-2.5 rounded-full bg-white/5 hover:bg-white/15 border border-white/10 text-emerald-400 backdrop-blur-md transition-all duration-300 hover:scale-105 active:scale-95"
        >
      ...
      <div className="w-full max-w-md z-10 flex flex-col items-center">
        ...
        <div className="relative mb-5 group">
          <div className="absolute -inset-1.5 bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full blur opacity-70 group-hover:opacity-100 transition duration-500" />
          <div className="relative w-24 h-24 rounded-full bg-[#0d1d22] border-2 border-emerald-400/40 p-2 flex items-center justify-center overflow-hidden shadow-2xl">
            {linksData?.logo_url ? (
              <img
        ...
        <div className="text-center mb-8 px-4">
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white mb-1.5 flex items-center justify-center gap-2">
            <span>
          ...
          <p className="text-xs sm:text-sm font-semibold tracking-[0.25em] text-emerald-400/80 uppercase">
        ...
        {linksData?.section_title && (
          <div className="w-full flex items-center justify-center gap-4 mb-6">
            ...
            <span className="text-[11px] font-bold tracking-[0.2em] text-white/50 uppercase px-2">
              {linksData.section_title}
            </span>
            <div className="h-[1px] flex-1 bg-gradient-to-r from-transparent via-emerald-500/30 to-transparent" />
          </div>
        ...
        {/* Links Stack */}
        <div className="w-full space-y-3.5 px-1">
          {visibleLinks.length === 0 ? (
            ...
            visibleLinks.map((link: any) => (
              <a
                ...
                className={`group relative flex items-center justify-between p-3.5 sm:p-4 rounded-2xl bg-[#0f2127]/80 hover:bg-[#152a32] border border-emerald-500/15 hover:border-emerald-400/40 backdrop-blur-xl transition-all duration-300 shadow-lg hover:shadow-emerald-500/10 hover:-translate-y-0.5 ${
